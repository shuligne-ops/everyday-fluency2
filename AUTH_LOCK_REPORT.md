...
- `auth_recovery_clicked` отправляется перед очисткой данных;
...
2. Подложен заведомо повреждённый ключ сохранённой auth-сессии. Вместо обычной страницы появился экран «Не удалось загрузить урок» с одной кнопкой «Восстановить доступ».
3. После нажатия кнопки экран исчез, обычная страница и список уроков загрузились без ошибок консоли.
...
5. Нажать «Восстановить доступ». Ожидается цель `auth_recovery_clicked`, очистка хранилищ и перезагрузка. После этого войти заново и открыть урок.
6. В Яндекс.Метрике проверить, что у `auth_error` есть `stage`, `reason`, `message`, а `message` не длиннее 200 символов.
