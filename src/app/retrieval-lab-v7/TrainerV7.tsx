  ...
  return <main className={styles.root}><div className={styles.wrap}>
    ...
    {!ready ? <p>Загружаю твою практику…</p> : !inSession || !session || !task || !pattern ? <>
      ...
      {(!session.result || action === 'technical') && <div className={styles.actions}>
        <button className={styles.mic} aria-label={voice.state === 'listening' ? 'Завершить запись' : 'Записать ответ'} disabled={checking || ['requesting', 'transcribing'].includes(voice.state)} onClick={() => voice.state === 'listening' ? voice.stop() : void voice.start()}>
