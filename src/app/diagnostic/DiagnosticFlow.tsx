...
import type { ClientScenario } from '@/lib/speaking-engine'
...
type Props = {
  move: string
  scenarios: { try: ClientScenario; retry: ClientScenario; transfer: ClientScenario }
}
...
const KICKER: React.CSSProperties = { fontSize: 13, letterSpacing: 1, textTransform: 'uppercase', color: '#f59e0b', fontWeight: 700, margin: 0 }

/** Первый абзац сцены — заголовок, остальные — тело. Так одна колонка обслуживает
 *  и короткий заголовок шага, и сцену с описанием. */
function splitScene(situation: string): { head: string; body: string[] } {
  const parts = situation.split('\n\n').map((p) => p.trim()).filter(Boolean)
  return { head: parts[0] ?? '', body: parts.slice(1) }
}

export default function DiagnosticFlow({ move, scenarios }: Props) {
  ...
  const tryScene = splitScene(scenarios.try.situation)
  const retryScene = splitScene(scenarios.retry.situation)
  const transferScene = splitScene(scenarios.transfer.situation)
  ...
  async function handleRetry(data: VoiceResult) {
    ...
    try {
      const analysis = await postEval('/api/diagnostic-retry-eval', { retry_session_id: data.session_id, anon_id: anonId, move })
  ...
  return (
    <main style={{ maxWidth: 640, margin: '0 auto', padding: '56px 24px', fontFamily: 'system-ui, sans-serif', color: '#0f1b3d' }}>
      {scenarios.try.screenTitle && <p style={KICKER}>{scenarios.try.screenTitle}</p>}
      <h1 style={{ fontSize: 24, lineHeight: 1.3, fontWeight: 700, margin: '14px 0 20px' }}>{tryScene.head}</h1>
      {tryScene.body.map((paragraph, index) => (
        <p key={index} style={{ fontSize: 17, lineHeight: 1.6, color: '#1f2b4d', margin: scenarios.try.instruction || index < tryScene.body.length - 1 ? '0 0 8px' : '0 0 28px' }}>{paragraph}</p>
      ))}
      {scenarios.try.instruction && <p style={{ fontSize: 17, lineHeight: 1.6, color: '#1f2b4d', margin: '0 0 28px', fontWeight: 600 }}>{scenarios.try.instruction}</p>}

      {phase === 'try' && <VoiceRecorder situationShownAt={tryShownAt} attemptId={attemptId} step="try" move={move} onResult={handleTry} />}
      ...
      {(phase === 'retry' || phase === 'transfer' || phase === 'done') && <section style={{ marginTop: 36, paddingTop: 28, borderTop: '1px solid #e3e7f0' }}>
        {scenarios.retry.screenTitle && <p style={KICKER}>{scenarios.retry.screenTitle}</p>}
        <h2 style={{ fontSize: 21, margin: '10px 0' }}>{retryScene.head}</h2>
        {retryScene.body.map((paragraph, index) => <p key={index} style={{ lineHeight: 1.6, margin: '0 0 18px' }}>{paragraph}</p>)}
        {scenarios.retry.instruction && <p style={{ lineHeight: 1.6, margin: '0 0 18px' }}>{scenarios.retry.instruction}</p>}
        {phase === 'retry' && <VoiceRecorder situationShownAt={retryShownAt} attemptId={attemptId} step="retry" move={move} onResult={handleRetry} />}
      ...
      {(phase === 'transfer' || phase === 'done') && <section style={{ marginTop: 36, paddingTop: 28, borderTop: '1px solid #e3e7f0' }}>
        {scenarios.transfer.screenTitle && <p style={KICKER}>{scenarios.transfer.screenTitle}</p>}
        <h2 style={{ fontSize: 21, margin: '10px 0' }}>{transferScene.head}</h2>
        {transferScene.body.map((paragraph, index) => <p key={index} style={{ lineHeight: 1.6, margin: '0 0 18px' }}>{paragraph}</p>)}
        {scenarios.transfer.instruction && <p style={{ lineHeight: 1.6, margin: '0 0 18px' }}>{scenarios.transfer.instruction}</p>}
        {phase === 'transfer' && <VoiceRecorder situationShownAt={transferShownAt} attemptId={attemptId} step="transfer" move={move} onResult={handleTransfer} />}
